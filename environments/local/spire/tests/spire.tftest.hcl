...
# wired to the exact OpenBao mount/role the openbao unit created, and
# every non-server/agent subchart stays off. (helm_release.spire_crds existing and
...
run "vault_upstream_authority_wired_to_openbao_pr1_role" {
  ...
  assert {
    ...
    error_message = "the vault upstreamAuthority plugin must target the exact mount/role the openbao unit created: pki mount, kubernetes auth backend, spire_server role"
  }
  assert {
    # Live-verified regression guard: the chart's own default token
    # audience ("vault") does NOT match the openbao unit's OpenBao role,
    # which sets `audience = var.openbao_endpoint` — a mismatch here is a
    # 403 "invalid audience (aud) claim" at spire-server startup.
...
run "bundle_publisher_targets_zot_this_pr" {
  ...
  assert {
    # Live-verified regression guard: the agent's OWN bootstrap trust
    # format is independent from the server's publish format above, but
    # both read the same ConfigMap — a mismatch hangs the agent forever
    # on "could not parse trust bundle ... no such file or directory".
  ...
  assert {
    # Repointed cross-namespace into ns "zot" — zot's mTLS
...
run "controller_manager_is_on_for_declarative_registration" {
  ...
  assert {
    # Reverses an earlier explicit negative-space choice: a real consumer
    # (ci namespace's default ServiceAccount) now exists, and the chart's
    # own default ClusterSPIFFEID already covers it — declarative
    # registration over a hand-rolled entry-create script.
    condition     = strcontains(helm_release.spire.values[0], "\"controllerManager\":\n    \"enabled\": true")
    error_message = "spire-server.controllerManager.enabled must be true — needed for its default ClusterSPIFFEID, declarative registration of the ci-namespace consumer"
  }
}
