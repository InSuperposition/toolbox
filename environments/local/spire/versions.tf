...
# Both ship via one `helm_release` of the upstream `spire` umbrella chart
# — tofu-owned, same ADR-0016 test as OpenBao (a long-lived, key-bearing
# process with a one-time bootstrap, not a Flux-helper shape).
...
terraform {
  ...
  required_providers {
    ...
    # never via tofu-to-tofu state sharing (docs/adr/0025).
  }
}
