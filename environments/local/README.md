...
| `kyverno/imagevalidatingpolicy.yaml` | the `ImageValidatingPolicy` (`policies.kyverno.io/v1`) — keyed cosign, tlog ignored (OpenBao Transit, no Rekor), the approval pubkey read from a ConfigMap via `resource.get()`, the in-toto predicate-type gate + two CEL validations (signature/subject, then `verdict == "approved"` with a `has()` fail-close). Matches namespaces labelled `toolbox.dev/cv-frontend: approval-enforced` (M3's `frontend` ns must carry it) + the `cv-frontend*` image glob. Image source is the in-cluster **zot**, repointed from GHCR once zot became HTTPS-only and Kyverno's referrer discovery started working against it directly. |
| `kyverno/validatingpolicy-buildkit-build-posture.yaml` | the `ValidatingPolicy` (`policies.kyverno.io/v1`) — enforces the spike-proven rootless-BuildKit securityContext ceiling (exact `runAsUser`/`runAsGroup`/`runAsNonRoot`, `seccomp: Unconfined`, `allowPrivilegeEscalation`, capabilities exactly `drop:[ALL] add:[SETUID,SETGID]`) as an allow-list at admission, on any pod's container named `build` — vacuously true on every other container, so no separate Tekton-task matcher is needed. Matches namespaces labelled `toolbox.dev/build-posture: enforced` (`ci`'s namespace must carry it). Complements, doesn't replace, `ci/tests/build-pipeline/chainsaw-test.yaml`'s static assert that the Task manifest itself is this shape. |
...
| `tests/kyverno/build-pod-posture/chainsaw-test.yaml` | `[k8s]`-gated, same wrapper: `buildkit-build-posture` Ready, an exact-ceiling `build` container is admitted, a non-`build` container with a fully privileged securityContext is unaffected (proves the vacuous-true scoping), a `privileged: true` or extra-capability `build` container is denied with the policy message. Own subdirectory — chainsaw resolves one fixed test-file name per directory. |
| `scripts/kyverno-chainsaw.sh` / `scripts/kyverno-kubeconform.sh` | the `chainsaw-kyverno` / `kubeconform-kyverno` hk gate wrappers — one shared skip-gate checks every policy under `kyverno/tests/` is reconciled before either Test runs |
...
| `kyverno/tests/crd-schemas/validatingpolicy_v1.json` | vendored `ValidatingPolicy` v1 CRD schema for the same gate, same regeneration rule |
| `flux/tests/crd-schemas/*.json` | v1/v2 CRD schemas vendored from Flux 2.9.5 + flux-operator v0.59.0 for the `kubeconform-flux` gate — **regenerate on a bump** (`oras resolve` + `cosign verify` per the lock-file header; CRDs from `github.com/fluxcd/flux2/releases/download/v2.9.5/manifests.tar.gz`) |
