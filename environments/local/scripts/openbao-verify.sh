...
#        Matches a `vault_mount` resource's `path = "transit"`
...
#        comment, or a DIFFERENT mount (e.g. `pki`, the mount SPIRE uses
#        for its upstream authority — a new mount with no
#        key-preservation constraint).
