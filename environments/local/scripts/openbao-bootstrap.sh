...
#
# After the key-preserved assertion it runs Phase C: the
...
#
# Acceptance (manual, needs a cluster) — bundle-only recovery:
#   rm -rf "$OPENBAO_STATE_DIR"/{root.token,seal.key,cluster,openbao.tfstate} \
...
#
# Test seams (openbao-bootstrap.bats):
#   TOOLBOX_OPENBAO_STATE_DIR            state dir (seal.key, root.token, snapshots/)
#   TOOLBOX_OPENBAO_KUBE_CONTEXT kube-context (default orbstack)
#   TOOLBOX_OPENBAO_ENDPOINT     in-cluster HTTPS endpoint
#   TOOLBOX_OPENBAO_LOCK         chart lock path (passed to the verify step)
...
ENDPOINT="${TOOLBOX_OPENBAO_ENDPOINT:-https://openbao.openbao.svc.cluster.local:8200}"
...
#: the `sops` Transit key, the decrypt-only policy, the
...
# `transit` mount or `approval-key` (openbao-verify.sh enforces
...
"$SCRIPT_DIR/openbao-verify.sh"

# ── 8. Phase A: the tofu-owned helm_release ──────────────────────────
# Migrate a legacy state-file name (openbao-cluster.tfstate -> openbao.tfstate,
# the unit rename) in place.
if [ ! -s "$TFSTATE" ] && [ -s "$STATE_DIR/openbao-cluster.tfstate" ]; then
	echo "==> Migrating $STATE_DIR/openbao-cluster.tfstate -> $TFSTATE"
	mv "$STATE_DIR/openbao-cluster.tfstate" "$TFSTATE"
	[ -f "$STATE_DIR/openbao-cluster.tfstate.backup" ] &&
		mv "$STATE_DIR/openbao-cluster.tfstate.backup" "$STATE_DIR/openbao.tfstate.backup"
fi
...
# ── 14. Phase C — API config against the restored instance ─
phase_c_apply
finish
