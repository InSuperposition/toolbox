...
# environments/local/scripts/spire-chainsaw.sh — the [k8s] skip
# gate. A fake kubectl on PATH drives the skip decision; these cover only
# that. The real end-to-end chainsaw run is the hk `chainsaw-spire`
# step (skips in CI, runs against the live cluster locally) and the
# documented `mise run local:spire:bootstrap` acceptance run.

setup() {
	load helper
	SW="$(toolbox_repo_root)/environments/local/scripts/spire-chainsaw.sh"
	...
	cat >"$FAKEBIN/kubectl" <<-'SH'
		...
		*"get statefulset spire-server"*) exit "${STUB_STS_RC:-0}" ;;
		*) exit 0 ;;
		esac
	SH
	...
	PATH="$FAKEBIN:$PATH"
	export TOOLBOX_SPIRE_KUBE_CONTEXT=orbstack
}

@test "TOOLBOX_SPIRE_SKIP_CHAINSAW forces the skip path, exit 0" {
	TOOLBOX_SPIRE_SKIP_CHAINSAW=1 run "$SW"
	[ "$status" -eq 0 ]
...
@test "cluster up but in-cluster SPIRE not deployed -> skip naming the bootstrap task" {
	STUB_STS_RC=1 run "$SW"
	[ "$status" -eq 0 ]
	[[ "$output" == *"local:spire:bootstrap"* ]]
}
