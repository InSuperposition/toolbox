...
# hk `chainsaw-openbao-cluster` step — the [k8s]-gated running-state check
# for the in-cluster OpenBao (T7c Increment 4b). Same gate shape as
...
#     (openbao-cluster-bootstrap.sh has not run on this cluster yet)
#   - otherwise: `chainsaw test` over environments/local/tests/openbao-cluster/
#
# It asserts the RUNNING post-migration state; it does NOT run the bridge or
# tear anything down.
#
# Test seam: TOOLBOX_OPENBAO_CLUSTER_SKIP_CHAINSAW=1 forces the skip path.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TESTS_DIR="$(cd "$SCRIPT_DIR/../tests/openbao-cluster" && pwd)"
CONTEXT="${TOOLBOX_OPENBAO_CLUSTER_KUBE_CONTEXT:-orbstack}"

skip() {
	echo "openbao-cluster-chainsaw: $* — skipping ([k8s] gate)"
	exit 0
}

[ -z "${TOOLBOX_OPENBAO_CLUSTER_SKIP_CHAINSAW:-}" ] || skip "TOOLBOX_OPENBAO_CLUSTER_SKIP_CHAINSAW set"
...
kubectl --context "$CONTEXT" -n openbao get statefulset openbao >/dev/null 2>&1 ||
	skip "in-cluster OpenBao not deployed (mise run local:openbao-cluster:bootstrap)"
