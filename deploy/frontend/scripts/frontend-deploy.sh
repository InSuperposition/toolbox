...
# `mise run frontend:deploy -- <registry/repo@sha256:...> <sha256:attestation-digest>`
#
# The consumer-side deploy gate for the local cv_frontend demo
...
if [ $# -ne 2 ]; then
	echo "usage: mise run frontend:deploy -- <registry/repo@sha256:<image>> <sha256:<attestation-digest>>" >&2
	exit 2
fi
IMAGE_REF="$1"
ATT_DIGEST="$2"

# --- 1. Verify (attestation/ seam, via $TOOLBOX_ATTESTATION_VERIFY). Nothing
#        is touched unless this passes. ---
frontend_attestation_verify "$IMAGE_REF" "$ATT_DIGEST" || {
	echo "frontend-deploy: verification failed — deployment unchanged" >&2
	exit 1
}
...
mv -f "$tmp" "$STATE"
echo "frontend-deploy: recorded $IMAGE_REF"
...
while [ "$(date +%s)" -lt "$deadline" ]; do
	...
	if [ -n "$code" ] && [ "$code" != "000" ]; then
		echo "frontend-deploy: frontend serving on :${HOST_PORT} (HTTP ${code})"
		exit 0
	fi
	sleep 1
done

echo "frontend-deploy: frontend did NOT come ready on :${HOST_PORT} within ${READY_TIMEOUT}s" >&2
