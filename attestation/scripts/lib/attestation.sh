...
# zot registry: HTTPS, no registry auth (unlike GHCR), but on a dev CA the
# host trust store does not carry — needs an explicit CA file, not the
# loopback --plain-http path. zot is shared toolbox infrastructure
...
# registry: a plain persistent file, already trusted by that same
# registry's other host-side caller, ci/scripts/registry-seed.sh's
# --to-ca-file. Go's x509 on darwin ignores SSL_CERT_FILE, so both
