...
# ci/scripts/chainsaw-test.sh — the [k8s] skip gate. Every case runs a fake
...
@test "TOOLBOX_CI_SKIP_CHAINSAW forces the skip path, exit 0" {
	TOOLBOX_CI_SKIP_CHAINSAW=1 run "$CI_SCRIPTS/chainsaw-test.sh"
	[ "$status" -eq 0 ]
	[[ "$output" == *"skipping ([k8s] gate)"* ]]
}

@test "no reachable cluster -> skip, exit 0" {
	STUB_KUBECTL_CLUSTERINFO_RC=1 run "$CI_SCRIPTS/chainsaw-test.sh"
	[ "$status" -eq 0 ]
	[[ "$output" == *"no reachable"* ]]
}

@test "cluster up but Tekton absent -> skip naming local:tekton:install" {
	STUB_KUBECTL_TEKTON_RC=1 run "$CI_SCRIPTS/chainsaw-test.sh"
	[ "$status" -eq 0 ]
