...
| `tasks/gate.yaml` | The blocking CRITICAL gate. One step: `trivy convert --scanners=vuln --exit-code=2 --severity=CRITICAL --ignorefile=$(params.IGNOREFILE) --format=table scan.json` over the `shared` workspace. `--exit-code=2` (not 1) so a CRITICAL match and a trivy error are distinguishable — `frontend-build.sh` shows the loud override box only on exactly 2. No `disable-ipv6` step (reads local files only, no network → nothing privileged). `IGNOREFILE` (default `/dev/null`) is the one param, never a consumer name: a caller points it at a git-committed `.trivyignore` to suppress specific CVE IDs statically — the exception is reviewed code, replayed identically, never a live vulnerability-DB call. `deploy/frontend/vex/.trivyignore` is the cv_frontend consumer's copy, committed empty. |
...
| `runtime/namespace.yaml` | the `ci` Namespace — nothing else. No `Role`/`RoleBinding`: the steps touch a mounted workspace + a registry, never the k8s API (**corrected 2026-09-14**: no TaskRun actually sets `automountServiceAccountToken: false` — none of them need the k8s API regardless of default automount). |
...
| `runtime/provenance-signer-sa.yaml` | T8 — the `provenance-signer` ServiceAccount, bound to only the `provenance-sign` PipelineTask (via `taskRunSpecs`, not the whole PipelineRun). Default automount — it needs its own projected token to authenticate to OpenBao's k8s-auth method. No RBAC. |
| `scripts/registry-seed.sh` | host-side: `oras cp` every `# syntax=` / `FROM …@sha256:` ref in a Dockerfile into the local zot, digests preserved, so the mirror has the base images. `mise run frontend:seed`. Consumer-agnostic — the Dockerfile is an argument. Not `crane` (R1b-ii-c pre-plan): `crane` has no CA-file override on this darwin/Go toolchain, so it can't make the plain-HTTP → HTTPS-with-dev-CA swap R1b-ii-c needs; `oras`'s `--to-ca-file` is independently scoped per endpoint. |
